...
def create_full_infor_user(
    ...
    persistent_roles = []
    for r in roles:
        role = Role.query.filter(Role.name.__eq__(r)).first()
        if role:
            persistent_roles.append(role)
